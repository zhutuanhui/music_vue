  ...
  <div class="app-container" ref="appcontainer">
    <el-card class="box-card">
      ...
      <el-table v-loading="loading" border :data="roleList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="歌单链接" prop="paly_url" :show-overflow-tooltip="true" width="150" />
        <el-table-column label="歌单id" prop="play_list_id" :show-overflow-tooltip="true" width="150" />
        <el-table-column label="要执行的播放次数" prop="cron_play_count" align="" width="100" />
        <el-table-column label="添加时播放次数" prop="old_play_count" :show-overflow-tooltip="true" width="150" />
        <el-table-column label="新歌单播放次数" prop="paly_count" width="100" />
          ...
          <el-row>
            <!-- <el-col :span="24">
              <el-form-item label="用户id" prop="uid" style="width: 90%">
                <el-input v-model="form.uid" placeholder="请输入用户名称" />
              </el-form-item>
            </el-col> -->
            <el-col :span="24">
              <el-form-item label="歌单链接" prop="paly_url" style="width: 90%">
                <el-input v-model="form.paly_url" placeholder="请输入用户名称" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="歌单id" prop="play_list_id" style="width: 90%">
                <el-input v-model="form.play_list_id" placeholder="请输入用户昵称" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="播放次数" prop="music_num" style="width: 90%">
                <el-input v-model="form.cron_play_count" placeholder="请输入cookie" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
...
<script>
import { listMusicPlaylist, getMusicPlaylistInfo, delMusicPlaylist, addMusicPlaylist, updateMusicPlaylist } from '@/api/music/playlist'
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listMusicPlaylist(this.addDateRange(this.queryParams, this.dateRange)).then(
        response => {
    ...
    handleSelectionChange(selection) {
      ...
      this.Playlist = selection.length !== 1
    ...
    handleUpdate(row) {
      ...
      getMusicPlaylistInfo(id).then(response => {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id !== undefined && this.form.id !== 0) {
            updateMusicPlaylist(this.form).then(response => {
            ...
            addMusicPlaylist(this.form).then(response => {
              if (response.code === 20000) {
                Message.success(response.msg)
                this.open = false
                this.addShow = false

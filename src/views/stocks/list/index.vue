  ...
  <div class="app-container" ref="appcontainer">
    <el-card class="box-card">
      <el-form ref="queryForm" :model="queryParams" :inline="true">
        <el-form-item label="名称" prop="name">
            <el-select v-model="queryParams.name" placeholder="请选择" style="width: 100%" @input="handlQueryName" clearable>
              <el-option clearable
                v-for="item in Stocks_type_name"
                :key="item.name"
                :label="item.name"
                :value="item.name"
              />
    ...
    <el-date-picker
      ...
      <el-table v-loading="loading" border :data="roleList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="股票名" prop="name" align="center" :show-overflow-tooltip="true" width="150" />
        <el-table-column label="股票代码" prop="code" align="center" :show-overflow-tooltip="true" width="150" />
        ...
        <el-table-column label="市值" prop="market_value" align="center" :show-overflow-tooltip="true" width="150"/>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope"> 
             <el-button
              ...
              type="text"
              icon="el-icon-edit"
              @click="handleDailyAdd(scope.row)"
            >每日股价</el-button>
            <el-button
              size="mini"
              type="text"
      ...
      <el-dialog :title="title" :visible.sync="open"  :width="dialogWidth">
        <el-form ref="form" :model="form" :rules="rules" label-width="80px">
          <el-row>
            <el-col :span="24">
              <el-form-item label="股票名" prop="name" style="width: 90%">
                <el-input v-model="form.name" placeholder="请输入账单名称" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="股票代码" prop="code" style="width: 90%">
                <el-input v-model="form.code" placeholder="请输入账单名称" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
            ...
            <el-col :span="24">
              <el-form-item label="市值" prop="market_value" style="width: 90%">
                <el-input v-model="form.market_value" placeholder="请输入市值" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
             ...
             <el-col :span="24">
              <el-form-item label="详情" prop="content" style="width: 90%">
                <el-input v-model="form.content"  type="textarea" placeholder="请输入详情" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
        ...
        <!-- 添加或修改用户 -->
      <el-dialog :title="title" :visible.sync="openDaily"  :width="dialogWidth">
        <el-form ref="formDaily" :model="formDaily" :rules="rules" label-width="80px">
          <el-row>
            <el-col :span="24">
              <el-form-item label="股票id" prop="stocks_id" style="width: 90%">
                <el-input v-model="formDaily.stocks_id" placeholder="请输入账单名称" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="现价" prop="money" style="width: 90%">
                <el-input v-model="formDaily.money" placeholder="请输入现价" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="最高价" prop="max_money" style="width: 90%">
                <el-input v-model="formDaily.max_money" placeholder="请输入最高价" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="最低价" prop="min_money" style="width: 90%">
                <el-input v-model="formDaily.min_money" placeholder="请输入最低价" />
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="涨跌百分比" prop="percent" style="width: 90%">
                <el-input v-model="formDaily.percent" placeholder="请输入市值" />
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button type="primary" @click="submitDailyForm">确 定</el-button>
          <el-button @click="cancel">取 消</el-button>
        </div>
      </el-dialog>
...
<script>
import { getStockslist, getStocksInfo, addStocks, updateStocks,delStocks } from '@/api/stocks/stocks'
import {addStocksDaily } from '@/api/stocks/daily'
...
export default {
 ...
 updated(){
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      // getStockslist(this.addDateRange(this.queryParams, this.dateRange)).then(
      getStockslist(this.queryParams).then(
        response => {
  ...
  /** 获取角色选项 */
    getStocksMoney() {
      getMoney(this.queryParams).then(response => {
        this.totalMoney = response.data.balance
        this.incomeMoney = response.data.income
        this.payMoney = response.data.pay
      })
    },
   /** 获取角色选项 */
    getAllAuthor(val) {
    ...
    handleDailyAdd(row) {
      this.reset()
      this.formDaily.stocks_id = row.id
      this.addShow = true
      this.editShow = false
      this.openDaily = true
      this.title = '添加股票'
    ...
    handleUpdate(row) {
      ...
      getStocksInfo(id).then(response => {
    ...
    handleDetail(row) {
      ...
      const id = row.id || this.ids
      this.$router.push({
        path:'/Stocks/Stocks-detail',
        query: {
          id: id,
          name: row.Stocks_title,
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          if (this.form.id !== undefined && this.form.id !== 0) {
            updateStocks(this.form).then(response => {
            ...
            addStocks(this.form).then(response => {
              if (response.code === 20000) {
                Message.success(response.msg)
                this.open = false
                this.addShow = false
    ...
    submitDailyForm: function() {
      this.$refs['formDaily'].validate(valid => {
        if (valid) {
            addStocksDaily(this.formDaily).then(response => {
              if (response.code === 20000) {
                Message.success(response.msg)
